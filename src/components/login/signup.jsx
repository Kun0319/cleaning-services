  ...
  const handleSubmit = async (e) => {
    ...
    if (true) {
      try {
        ...
        // 如果驗證都通過才可以送出註冊
        if (upname && upbirthday && upphone && checkCode && upid && uppassword) {
      ...
      >
        <ul>
          ...
          <li className="loginli">
            ...
            <input type='button' value='發送驗證碼' id='checkUserEmail' disabled={workMode} onClick={startCountdown} ></input>
          ...
          <li className="loginli">
            ...
            <label htmlFor="veriCode" className='myLoginLabel'>驗證碼</label>
            <input type="text" id='veriCode' onInput={(e) => setUserCode(e.target.value)} onChange={
              async () => {
                let checkResult = await checkVeriCode(userCode, upemail);
