...
const UploadClearImg = ({ empClearData }) => {
    ...
    return (
        <form>
            ...
            {imageData && imageData.map((file, index) =>{
                    if((index + 1) % 2 !== 0){
                        return (<div key={index} className='clearImgDiv'>
                            <span>清掃前</span><img onClick={handleClickImg} src={file.url} style={{cursor:"pointer"}} className="orderImages w-100" alt="orderImages" />
                                </div>)
                    }else{
                        return <div key={index} className='clearImgDiv2'><span>清掃後</span><img onClick={handleClickImg} src={file.url} style={{cursor:"pointer"}} className="orderImages w-100" alt="orderImages" /></div>
                    }
                })}
            </div>
                <div className='w-100'>
                <button className='orderBtn' type="button" onClick={() => {handleImagesUp()}}>打掃完成</button>
                <span className='me-0'  style={{border:"1px solid black",borderRadius:"3px",padding:"2px"}}>現在時間:{dateTime}</span>
                </div>
            </form>
    )    
}
